...
export default class IPv6{
    ...
    constructor(ip:number[] | "random" = "random", cidr:number=32){
        ...
        let info = getCalculatedInfo(this.ipArray, this.cidr)
        this.netid=info.netid
        this.firstHost=this.netid
        this.lastHost=info.lastHost
    ...
    getRandomSubnet(): V6OutputSubnetInfo{
        let newCidr = getRandomIntInclusive(this.cidr+1, 64)
...
// we just subnet on the 32bit before the hostpart -> so its not the real subnetmask etc->partNetid partFirsthost etc...
function getCalculatedInfo(ipArray:number[], cidr:number){
    ...
    let partIp = ipArray[2]*0x10000+ipArray[3]
    ...
    function pushIntoArray(twoBlocks:number, ipArray: number[]){
        let highBlockBits = twoBlocks / 65536.0
        let lowBlockBits = twoBlocks % 65536.0
